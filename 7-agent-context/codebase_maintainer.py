...
两种用法：
1. 被 `06_three_day_workflow.py` 导入（推荐，可按天分阶段跑）
2. 直接运行本文件 —— 会依次执行 explore() / analyze() / plan_next_steps() / generate_report()，
   共 3 次 Agent 调用，探索目标为同目录下的 `codebase/`
"""
...
# [本地修复] 上游此文件本身没有加载 .env：它是设计成被 06_three_day_workflow.py 导入使用的，
# 环境变量靠 06 里的 load_dotenv() 顺带带进来。单独运行本文件（PyCharm 右键 Run 等）时，
# 进程里没有 LLM_API_KEY / LLM_BASE_URL，构造 HelloAgentsLLM() 会直接抛：
#   HelloAgentsException: API密钥和服务地址必须被提供或在.env文件中定义。
# 框架是在 __init__ 里才读 os.getenv（core/llm.py:83），所以在这里加载就来得及。
...
# [本地修复] 锚定本文件所在目录，避免路径随「当前工作目录」漂移
# （PyCharm 右键运行时 cwd 常是项目根，而非脚本所在目录）
...
"""
    ...
    def generate_report(self, save_to_file: bool = True) -> Dict[str, Any]:
        ...
        if save_to_file:
            # [本地修复] 上游是相对当前工作目录的 f"maintainer_report_{session_id}.json"，
            # 从别处运行时报告会落到那个目录里。改为锚定本文件所在目录。
            report_file = os.path.join(_SCRIPT_DIR, f"maintainer_report_{self.session_id}.json")
...
def main():
    ...
    # [本地修复] 上游此处是 project_name="my_flask_app" + codebase_path="./my_flask_app"，
    # 那是作者留给读者替换的占位路径。直接跑会有个隐蔽问题：TerminalTool.__init__ 里
    # 带 self.workspace.mkdir(parents=True, exist_ok=True)，所以它不会报错，
    # 而是**静默建出一个空的 my_flask_app/ 目录**，Agent 最后去探索一个空文件夹。
    # 这里改为用同目录下真实存在的 codebase/（与 06_three_day_workflow.py 保持一致）。
    maintainer = CodebaseMaintainer(
        project_name="demo_codebase",
        codebase_path=os.path.join(_SCRIPT_DIR, "codebase"),
        llm=HelloAgentsLLM()
    )
